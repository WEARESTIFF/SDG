...
function custom_theme_options() {
  ...
  $custom_settings = array( 
    ...
    'sections'        => array( 
      ...
      array(
        'id'          => 'global',
        'title'       => '<i class="dashicons-before dashicons-admin-site"></i>&nbsp; &nbsp; ' . __( 'Global', HB_DOMAIN_TXT )
      ),
    ...
    'settings'        => array(
      ...
      array(
        ...
        //'desc'        => sprintf( __( 'The Background option type is for adding background styles to your theme either dynamically via the CSS option type below or manually with %s. The Background option type has filters that allow you to remove fields or change the defaults. For example, you can filter %s to remove unwanted fields from all Background options or an individual one. You can also filter %s. These filters allow you to fine tune the select lists for your specific needs.', HB_DOMAIN_TXT ), '<code>ot_get_option()</code>', '<code>ot_recognized_background_fields</code>', '<code>ot_recognized_background_repeat</code>, <code>ot_recognized_background_attachment</code>, <code>ot_recognized_background_position</code>, ' . __( 'and', HB_DOMAIN_TXT ) . ' <code>ot_type_background_size_choices</code>' ),
        'desc'        => '',
        'std'         => '',
        'type'        => 'upload',
      ...
      array(
        ...
        // 'desc'        => __( 'You can change site title from: Settings / General', HB_DOMAIN_TXT ),
        'std'         => '',
      ...
      array(
        'id'          => 'back_to_blog',
        'label'       => __( 'Back to Blog Target', HB_DOMAIN_TXT ),
        // 'desc'        => sprintf( __( 'The On/Off option type displays a simple switch that can be used to turn things on or off. The saved return value is either %s or %s.', HB_DOMAIN_TXT ), '<code>on</code>', '<code>off</code>' ),
        'std'         => get_option('page_for_posts'),
        'type'        => 'page-select',
        'section'     => 'global',
        'rows'        => '',
        'post_type'   => '',
      ...
      array(
        'id'          => 'footer_social_active',
        'label'       => __( 'Footer Social Buttons', HB_DOMAIN_TXT ),
        // 'desc'        => sprintf( __( 'The On/Off option type displays a simple switch that can be used to turn things on or off. The saved return value is either %s or %s.', HB_DOMAIN_TXT ), '<code>on</code>', '<code>off</code>' ),
        'std'         => 'off',
        'type'        => 'on-off',
        'section'     => 'footer',
      ...
      array(
        'id'          => 'footer_copywright',
        'label'       => __( 'Copyright', HB_DOMAIN_TXT ),
        // 'desc'        => sprintf( __( 'The On/Off option type displays a simple switch that can be used to turn things on or off. The saved return value is either %s or %s.', HB_DOMAIN_TXT ), '<code>on</code>', '<code>off</code>' ),
        'std'         => '',
        'type'        => 'text',
        'section'     => 'footer',
 ...
 * @since     2.0
 */
if ( ! function_exists( 'ot_type_uniq_id' ) ) {
  
  function ot_type_uniq_id( $args = array() ) {
    
    /* turns arguments array into variables */
    extract( $args );

    if ( empty($field_value) ) { 
      $hb_answer_counter = get_option('hb_answer_counter');
      $hb_answer_counter++;
      $field_value = $hb_answer_counter;
      update_option('hb_answer_counter', $hb_answer_counter);
    }
    ...
    echo '<div class="format-setting type-uniq-id ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
        ...
        /* build text input */
        echo '<div style="padding: 0 0 0 10px;font-weight: bolder;font-size: 1.3em;">#',$field_value, '</div>';
        echo '<input type="hidden" name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $field_id ) . '" value="' . esc_attr( $field_value ) . '" class="widefat option-tree-ui-input ' . esc_attr( $field_class ) . '" readonly/>';
